## Endless Snow
  - A Minecraft plugin to allow endless snow pile-up and persistent storms
    ...
    - Supports Spigot 26.1.2
    - Other versions may be supported by an older [release](https://github.com/stuarthayhurst/endless-snow-plugin/releases)
      - The earliest supported version was 1.21.10
...
## Datapack:
  - Optionally, the frozen biomes datapack can be used
    - It adds the `winter-base` world preset that only includes snowy biomes
  ...
  - It must be installed _before_ world generation, and used as the world preset
    - For servers, set `level-type` to `winter-base` in `server.properties`
  - `winter-base` sets the entire world to a frozen river

### `winter-full` preset:
  - To generate a world using all snowy biomes available, use `v1.2.0` on Minecraft 1.21.11
    - The datapack breaks on newer versions if it includes `winter-full`
  - To experiment with `winter-full` on newer versions, use `winter-full-archive`
    - This branch isn't expected to work, and currently just contains the final commit before its removal
...
## Installation:
  ...
  - Copy the plugin and (optionally) the datapack to your server
    - If the datapack is used, set `level-type` to `winter-base` in `server.properties`
